<?php
require_once($_SERVER['DOCUMENT_ROOT'].'/erp/PHPExcel.php');
require_once($_SERVER['DOCUMENT_ROOT'].'/erp/PHPExcel/IOFactory.php');
...
//include $_SERVER['DOCUMENT_ROOT'].'/erp/PHPExcel/IOFactory.php';
function SafeRequestParametersExcel($data)
{

	$jsondata = array();
	$file_name = $_FILES['Boleta']['name'];
	$ext = pathinfo($file_name, PATHINFO_EXTENSION);
	$file_name = $_FILES['Boleta']['tmp_name'];
	...
	$jsondatareserva=null;
	for ($row = 1; $row <= $highestRow; $row++) {
			//  Read a row of data into an array
		$rowData = $sheet->rangeToArray('A' . $row . ':' . $highestColumn . $row, 
			NULL, TRUE, FALSE);
			//echoing every cell in the serialize(value)lected row for simplicity. You can save the data in database too.
