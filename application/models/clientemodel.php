...
{
	public function cargarClientes($FILES)
	{
		$file_name = $FILES['Boleta']['name'];
		$ext = pathinfo($file_name, PATHINFO_EXTENSION);
		if($ext == "xlsx"){
			$jsondata=SafeRequestParametersExcel($FILES);
			$this->db->trans_start();		
			for ($i=0; $i < count($jsondata['data']) ; $i++){
				$ruc  = $jsondata['data'][$i][0];
				$dni  = $jsondata['data'][$i][1];
				$nombre  = $jsondata['data'][$i][2];
				$correo  = $jsondata['data'][$i][3];
				$telefono1  = $jsondata['data'][$i][4];
				$telefono2  = $jsondata['data'][$i][5];
				$direccion  = $jsondata['data'][$i][6];
				$idempresa  = $jsondata['data'][$i][7];
			...
			if ($this->db->trans_status() === FALSE)
			{
				...
				$this->responsemodel->message = 'Se cargo con exito los clientes';		
			}		
		}else{
			$this->responsemodel->SetResponse(false);	
			$this->responsemodel->message = 'El archivo que desea cargar no tiene la extensión .xlsx';				
		}		 	
